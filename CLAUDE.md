...
9. **One build per session.** Read `builds.md` first; take the next sequential build numbers. Commit format `B{NNN}: description`. Update `builds.md`, `tasks.md`, and `ROADMAP.md` at the build boundary.
...
| `teleport.js` | Jump the Cesium scene to a lat/lon or named landmark (`--list`); host from routes.json | Unreal running |
