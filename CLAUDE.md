...
- **lib/texturePromptBuilder.js** - Generates Meshy API prompts from building metadata. Pure functions, no API calls — fully testable offline. Uses architecture style classification (materials, decorative elements, weathering, era exclusions) to build texture prompts. Quality tiers (hero/foreground/background/distant) control polycount. Exports `buildTexturePrompt()`, `buildNegativePrompt()`, `buildPromptsForBuilding()`, `previewAllPrompts()`, `QUALITY_TIERS`
