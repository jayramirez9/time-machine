...
- **lib/geminiImageGen.js** - Google Gemini image generation API client. Generates architectural reference images from text prompts for the Meshy Image-to-3D pipeline. Requires `GOOGLE_AI_API_KEY` env var. Exports `generateImage()`, `saveImage()`, `toDataUri()`
- **lib/meshImport.js** - Import Meshy-generated FBX/GLB into Unreal via RC API. Reads GENERATION_MANIFEST.json, matches to building footprint positions, generates Python scripts that download meshes from daemon HTTP server, import into UE Content Browser, and spawn StaticMeshActors with PBR textures. Exports `manifestToSpawnData()`, `buildMeshImportScript()`, `buildMeshClearScript()`, `ACTOR_PREFIX`
- **lib/propCatalog.js** - Era-appropriate street furniture definitions. 16 prop types (hitching posts, fire hydrants, awnings, mailboxes, telegraph/telephone poles, parking meters, traffic lights, etc.) with introduction/removal years, placement rules (sidewalk/intersection/facade), spacing, density weights, scale. Exports `PROPS`, `getPropsForYear()`, `getPropsForRoad()`, `getPropsByPlacement()`, `summarizePropsForYear()`
- **lib/propPlacement.js** - Street prop placement engine. Walks road splines placing props at configured intervals, places intersection props at crossroads, de-duplicates within 3m radius, uses deterministic seeded PRNG for consistent output. Supports `--only`/`--exclude` filtering. Exports `placeProps()`, `placePropsDryRun()`, `buildPropSpawnScript()`, `PROP_PREFIX`
- **lib/texturePromptBuilder.js** - Generates Meshy API prompts and Gemini reference image prompts from building metadata. Pure functions, no API calls — fully testable offline. Uses architecture style classification (materials, decorative elements, weathering, era exclusions) to build texture prompts and reference image prompts. Quality tiers (hero/foreground/background/distant) control polycount. Exports `buildTexturePrompt()`, `buildNegativePrompt()`, `buildPromptsForBuilding()`, `previewAllPrompts()`, `buildReferenceImagePrompt()`, `buildReferenceImageForBuilding()`, `QUALITY_TIERS`
...
- **test/propPlacement.test.js** - Unit tests for prop catalog data integrity, era filtering, road category filtering, placement engine, determinism, intersection props, spawn script generation
- **test/geminiImageGen.test.js** - Unit tests for Gemini image generation client (mocked fetch)
- **test/meshImport.test.js** - Unit tests for mesh import manifest parsing, spawn data generation, Python script generation
...
- **tools/preview-textures.js** - **Texture prompt preview tool.** Reads buildings.geojson, classifies each building, generates texture prompts — zero API calls, zero credits. For tuning prompts before spending Meshy credits. Supports `--summary` (style distribution, credit estimate), `--index N` (single building), `--json`, `--quality hero|foreground|background|distant`. Usage: `node tools/preview-textures.js terrain-data/manhattan-ny/ --era nyc_1884 [--summary]`
- **tools/generate-building-refs.js** - **Reference image → 3D building generator.** Two-stage pipeline: Gemini generates an architectural reference image from building metadata, then Meshy Image-to-3D converts it to a textured 3D mesh. Tier 2 in the asset pipeline — used when no historical photo exists but you want better than text-only. Supports `--image-only` (Gemini only, no Meshy), `--dry-run`, `--index N`, `--quality`, `--force`. Output: reference images to `terrain-data/{slug}/reference-images/`, meshes to `mesh-data/{slug}-ref/`. Requires `GOOGLE_AI_API_KEY` + `MESHY_API_KEY`. Usage: `node tools/generate-building-refs.js terrain-data/manhattan-ny/ --era nyc_1884 [--image-only] [--dry-run]`
- **tools/spawn-meshes.js** - **Mesh import spawner.** Reads GENERATION_MANIFEST.json files from mesh-data/, matches to building footprints in buildings.geojson, imports FBX into Unreal Content Browser, and spawns textured StaticMeshActors at correct geo positions via RC API. Files transfer from Mac to PC via daemon HTTP server (same pattern as terrain import). Actor naming: `TM_Mesh_{idx}_{slug}`. Supports `--dry-run`, `--clear`, `--index N`, `--daemon-url`. Usage: `node tools/spawn-meshes.js terrain-data/manhattan-ny/ [--dry-run] [--daemon-url http://100.68.243.96:3000]`
- **tools/spawn-props.js** - **Street prop spawner.** Reads road splines, places era-appropriate street furniture (hitching posts, fire hydrants, awnings, mailboxes, traffic lights, etc.) along sidewalks and at intersections. Filters by year — 1884 gets hitching posts and horse troughs, 1978 gets parking meters. Actor naming: `TM_Prop_{type}_{idx}`. Supports `--dry-run`, `--clear`, `--only <types>`, `--exclude <types>`. Usage: `node tools/spawn-props.js terrain-data/manhattan-ny/ --year 1884 [--dry-run]`
