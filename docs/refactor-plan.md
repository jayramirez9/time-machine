...
## Accepted Trade-offs (Jay's call to reverse)

**Witness-window sequencing.** Roughly nine structural builds land before any *new* witness-scene content. The conformance set stays the three existing locales — defensible because the Unreal rebuild is happening regardless and `baton-rouge-1978` is itself a living-witness scene — but PRD §26's expiring-ground-truth argument would instead pull the photo-first pilot (Baton Rouge 1985, open product question 4 in `v3-pivot-plan.md`) forward. This plan defers that decision rather than answering it by omission. If decided now, the pilot slots after P6 without disturbing the contract builds.
...
2. **Parity before improvement.** Ports (P4, P6, P7) reproduce current behavior first, verified by replay. Enhancements are separate builds.
