...
- Suite green — **the pre-existing timezone flake that would have undermined this is already fixed.** PR #7 (`fix-worldstate-local-hour`) changed the compiler to derive the scene hour from `timestampLocal` instead of machine-local `getHours()` on the UTC instant, so the time-of-day classification this refactor's ecology gating reads is now TZ-deterministic (suite 1592/0 on a non-UTC box). Land PR #7 first so the ecology gating builds on a green, deterministic base.
