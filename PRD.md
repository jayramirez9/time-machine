...
│  MusicBrainz │ Spotify │ Apple Music │ NYT Archive       │
└─────────────────────────────────────────────────────────┘
```
...
**Primary data source: NYT APIs.** The New York Times provides the most complete structured news API available — Article Search API covers every article published since **1851**, with full metadata (headline, abstract, byline, section, keywords, pub date). The Archive API returns all articles for any given month/year back to 1851 in bulk. Top Stories and Most Popular APIs cover current news. Free tier: 500 requests/day. This serves double duty: (1) current news scanning for the business intelligence use case, and (2) historical newspaper archive for the Time Machine agent research layer (Phase 7) — pull the actual NYT front page for any day in history as primary source material for cultural context.
...
5. **The Cultural Agent** researches 1884 NYC: queries NYT Archive API for every article published that month — headlines reveal what people were talking about, advertisements reveal commerce and brands, weather reports corroborate NOAA data, police blotters reveal street life. Cross-references with infrastructure records (which streets had gas lamps, the elevated railway schedule, horse car routes). Outputs cultural metadata. For the music dimension, queries MusicBrainz for recordings available at the target date and cross-references published song catalogs (pre-recording era) to build a date-locked music profile.
...
8. **NYT Archive API for cultural research:** Test the Article Search and Archive APIs against specific Place×Time targets. Key questions: How rich is the metadata for pre-1900 articles? Can we extract street-level detail (vendor calls, infrastructure, commerce) from article text? What's the realistic pipeline from bulk article download → structured cultural metadata? Can advertisements be isolated and parsed for brand/product availability data? How does coverage vary for non-NYC locations (e.g., Baton Rouge 1978 — is there enough local detail, or is the NYT too NYC-centric for other markets)?
9. **MLS data as building source:** Evaluate MLS/real estate data access for building reconstruction. Key questions: API access (RETS/RESO Web API, Zillow, Realtor.com, Redfin), licensing restrictions on bulk parcel data, coverage completeness for year-built and square footage fields, photo availability and resolution by market, feasibility of automated grey-cube generation from structured listing data (footprint from lot + sq ft + stories), and AI photo→PBR texture extraction quality from typical MLS exterior photos (multiple angles but uncontrolled lighting/lens). Secondary question: can county assessor/tax records supplement or replace MLS for the structured fields (year built, sq ft, lot size) without the licensing complexity?
