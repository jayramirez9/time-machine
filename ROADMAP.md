...
> **PRD v3.0 (Flinch Standard, 2026-07-29) — what changed for this roadmap.** The governing law is now the flinch of a witness, not a citation; the confidence/citation apparatus is demoted to internal engineering metadata. Two roadmap consequences: **(1) content priority is living-witness-first** (PRD §26 — the ground truth is expiring: build scenes with people still alive to test them, before those scenes become merely-researched); **(2) a photo-first authoring pipeline** becomes the primary path where a dated frame exists (PRD §3.1) — see the new **Phase 10** below. The pending **provenance refactor** (accuracy manifest → provenance declaration; strip confidence from agent outputs; ecology diurnal/temperature gating) is scoped in `docs/scope-code-provenance-refactor.md`. The **full code-pivot roadmap** — all the places the code still runs the old constitution, ranked, with a sequenced build order — is in `docs/v3-pivot-plan.md` (from a multi-agent review of the codebase against v3.0, 2026-07-30). All Technology Watch content is retained. **The structural half of that pivot is now a ratified plan:** the Locale Package refactor — contract, loader, schedule channel, fresh UE 5.8.1 rebuild on the a7500, flinch eval harness — is sequenced in `docs/refactor-plan.md` (D001, `docs/architecture-v3.md`, `contracts/locale-package-v0.1.md`).
...
- [x] **Ion client corrections** *(done 2026-07-22; live-verify owed)*: the request is now built from ion's **OpenAPI spec** (`https://ion.cesium.com/openapi.yaml`) rather than from guesses — `type: 3DTILES` + `options.sourceType: 'RASTER_IMAGERY'` (the ImageryRasterOptions schema *is* the reconstruction job) + a **required** `options.outputs` array carrying `{ outputType: 'SPLATS_3DTILES' }` alongside the mandatory `3DTILES` mesh output. Without `outputs` the job cannot emit a splat at all, which the previous payload omitted entirely. Derived outputs are captured from the **create** response (spec field `assets`; `additionalAssets` in Cesium's own example — both merged and deduped), since `GET /v1/assets/{id}` carries no derived list. Identification is made deterministic by **naming the outputs ourselves** (`outputs[].name`, a spec-supported field) and tagging them `[SPLATS_3DTILES]`, so the splat is found by an exact string we chose rather than inferred; `identifySplatOutput()` reports *how* it matched (`type` | `tag` | `name`) and **refuses to guess** — including when a hint is merely inherited from the parent job name — rather than risk streaming a mesh into `TM_SplatTileset`. Also fixed: `options` rebuilt from `sourceType` alone (dropped `outputs`), a trailing `--source-type` blanking the default, and `--status` claiming a splat id it can't have. New `--dry-run` request-body preview, repeatable `--option k=v`, and `createAssetHint()` for ion's 404-means-missing-write-scope trap. +41 tests (1551 → 1592).
  > **Two wrong turns, recorded so they aren't repeated.** (1) The original `RAW_IMAGERY` appears in no Cesium source — fabricated. (2) A first pass at this fix replaced it with `3D_CAPTURE`, reasoning that `RASTER_IMAGERY` was imagery-only. Wrong: `RASTER_IMAGERY` is a **sourceType** (describes the raster *input*), while `IMAGERY` is an **AssetType** (a 2D overlay *output*) — conflating them inverted the conclusion. `3D_CAPTURE` expects "an OBJ, COLLADA, or glTF model created through photogrammetry processes", i.e. an already-reconstructed model, not a directory of JPEGs. The lesson worth keeping: prose guides don't cover this endpoint; **read `openapi.yaml`**.
- [ ] **Trinity re-run — live verification** *(blocked on Jay)*: needs an ion token with `assets:read`+`write`+`list`, asset-access ALL, in `.env`. Preview the exact body first (`--dry-run`, free), then `node tools/cesium-capture.js --photos photos/spike-trinity/ --name "Trinity Church 1903 (archival)" --no-wait`. Confirms three still-unverified things: that ion accepts this options shape, that `assets`/`additionalAssets` comes back populated, and which field actually marks the splat output (the spec shows no per-derived-asset output-type field, though its create-response example carries `"type": "LAS"` on a derived asset, which is why `type` is the first probe — the run confirms which marker is real, and the name tag is the fallback that does not depend on the answer).
...
- **UE scene → splat bake, streamed to the browser** ([PlayCanvas](https://blog.playcanvas.com/turning-a-gaussian-splat-into-a-videogame/), [Atlux](https://atlux.one/gaussian-splatting), SuperSplat) — the *opposite direction* from Phase 7d: not photos→splat for fidelity, but UE→splat for **distribution** (a finished scene as a self-contained static page, no install, works on a phone). Pipeline is camera volumes in UE → rendered viewpoints → COLMAP-format export → train in Postshot/LichtFeld. Not photogrammetry and it doesn't inherit that ceiling — poses are exact by construction, the point cloud is sampled from our own mesh geometry, frames render clean; better *input* than archival plates (though never better than the UE scene it photographs). Walkable rather than orbit-only: collision geometry can be derived from the splat via `splat-transform` (a voxel grid → `.collision.glb`, with seed position and an interior/exterior fill strategy to tune — not automatic). Licensing splits three ways: *delivery* (SuperSplat, `splat-transform`, supersplat-viewer, PlayCanvas engine) is **MIT and self-hostable**; *capture* (Atlux) is a **paid commercial UE plugin**; *training* is a third tool.
  > **What a bake actually costs: the lighting freezes — and so does everything that moves.** Not just WorldState-driven sun and weather (Law 5.1 One Universe; "Everything Hangs Off WorldState", `PRD.md:443`), but every dynamic system Phase 7b built: Niagara chimney smoke and lamp glow, 2200K gaslight flicker, wind-driven awnings and laundry, swinging signs, rain and snow particles, and any future crowd. Worse, 3DGS training assumes a *static* scene, so those systems reconstruct as floaters — you'd disable them before capture. **The scene doesn't just stop relighting; it stops moving.**
  >
