...
/**
 * Build the Python script that spawns road spline actors from GeoJSON data.
 * Downloads roads-splines.json from daemon, creates one Actor per road group
 * with a SplineComponent populated with control points.
 */
function buildSplineImportScript(daemonUrl, slug) {
...
# Download road spline data from Mac daemon
url = "${daemonUrl}/terrain-data/${slug}/roads-splines.json"
local_path = os.path.join(tmp, "tm_${safeSlug}_roads.json")
unreal.log(f"[TM] Downloading road splines from {url}")
urllib.request.urlretrieve(url, local_path)

with open(local_path) as f:
    roads = json.load(f)

unreal.log(f"[TM] Loaded {len(roads)} road spline groups")

world = unreal.EditorLevelLibrary.get_editor_world()
subsys = unreal.get_editor_subsystem(unreal.EditorActorSubsystem)

# Delete existing TM road actors
existing = subsys.get_all_level_actors()
for actor in existing:
    label = actor.get_actor_label()
    if label.startswith("TM_Road_"):
...
for i, road in enumerate(roads):
    ...
    # Spawn actor at first control point
    first = pts[0]
    loc = unreal.Vector(first[0], first[1], first[2] if len(first) > 2 else 10.0)
    actor = subsys.spawn_actor_from_class(unreal.Actor, loc)
    if not actor:
        unreal.log_warning(f"[TM] Failed to spawn actor for road {i}")
        continue

    cat = road.get("category", "road")
    actor.set_actor_label(f"TM_Road_{cat}_{i}")
    ...
    for j, pt in enumerate(pts):
        pos = unreal.Vector(pt[0], pt[1], pt[2] if len(pt) > 2 else 10.0)
        spline.add_spline_point_at_index(pos, j, unreal.SplineCoordinateSpace.WORLD, True)

    spline.set_spline_point_type(0, unreal.SplinePointType.CURVE, True)
    actor.set_root_component(spline)
    spline.register_component()
    spawned += 1

unreal.log(f"[TM] Spawned {spawned} road spline actors")
`.trim();
}

/**
 * Build the Python script that imports a landscape mask texture.
 * Downloads mask-{type}.png from daemon and imports as a texture asset.
 */
function buildMaskImportScript(daemonUrl, slug, maskType) {
  const safeSlug = slug.replace(/-/g, '_');
  return `
import unreal
import urllib.request
...
# Download mask from Mac daemon
mask_url = "${daemonUrl}/terrain-data/${slug}/mask-${maskType}.png"
mask_path = os.path.join(tmp, "tm_${safeSlug}_mask_${maskType}.png")
unreal.log(f"[TM] Downloading ${maskType} mask from {mask_url}")
urllib.request.urlretrieve(mask_url, mask_path)
unreal.log(f"[TM] Saved mask to {mask_path}")

# Import as texture asset
task = unreal.AssetImportTask()
task.set_editor_property("filename", mask_path)
task.set_editor_property("destination_path", "/Game/TimeMachine/Terrain")
task.set_editor_property("destination_name", "Mask_${maskType}_${safeSlug}")
task.set_editor_property("replace_existing", True)
task.set_editor_property("automated", True)
task.set_editor_property("save", True)
unreal.AssetToolsHelpers.get_asset_tools().import_asset_tasks([task])

tex = unreal.EditorAssetLibrary.load_asset("/Game/TimeMachine/Terrain/Mask_${maskType}_${safeSlug}")
if not tex:
    unreal.log_error("[TM] Failed to import ${maskType} mask texture")
else:
    unreal.log(f"[TM] ${maskType} mask texture imported: {tex.get_name()}")
`.trim();
}
...
/**
 * Import OSM vector data into Unreal (road splines + landscape masks).
 * Checks if vector data exists in terrain-data/{slug}/ and imports if found.
 *
 * @param {Object} opts
 * @param {string} opts.host - Unreal RC API host
 * @param {string} opts.daemonUrl - Mac daemon URL reachable from PC
 * @param {string} opts.slug - Location slug for terrain-data directory
 * @returns {Promise<{ok: boolean, splines?: number, masks?: string[], error?: string}>}
 */
export async function importVectors(opts) {
  const { host, daemonUrl, slug } = opts;
  const outputDir = path.join(TERRAIN_DIR, slug);
  const metadataPath = path.join(outputDir, 'metadata.json');

  // Check if vector data exists
  if (!fs.existsSync(metadataPath)) {
    return { ok: false, error: 'No metadata.json found' };
  }
  const meta = JSON.parse(fs.readFileSync(metadataPath, 'utf8'));
  if (!meta.vectors) {
    return { ok: false, error: 'No vector data in metadata — run tools/fetch-vectors.js first' };
  }

  const results = { ok: true, splines: 0, masks: [] };

  // Import road splines
  const splinesPath = path.join(outputDir, 'roads-splines.json');
  if (fs.existsSync(splinesPath)) {
    console.log(`[Vectors] Importing road splines...`);
    const script = buildSplineImportScript(daemonUrl, slug);
    const result = await runPython(host, script);
    if (result.ok) {
      results.splines = meta.vectors.splineCount || 0;
      console.log(`[Vectors] Road spline import script executed (${results.splines} groups)`);
    } else {
      console.warn(`[Vectors] Spline import failed (non-fatal): ${result.error}`);
    }
  }

  // Import mask textures (water, landuse, roads)
  for (const maskType of ['water', 'landuse', 'roads']) {
    const maskPath = path.join(outputDir, `mask-${maskType}.png`);
    if (fs.existsSync(maskPath)) {
      console.log(`[Vectors] Importing ${maskType} mask texture...`);
      const script = buildMaskImportScript(daemonUrl, slug, maskType);
      const result = await runPython(host, script);
      if (result.ok) {
        results.masks.push(maskType);
        console.log(`[Vectors] ${maskType} mask imported`);
      } else {
        console.warn(`[Vectors] ${maskType} mask import failed (non-fatal): ${result.error}`);
      }
    }
  }
