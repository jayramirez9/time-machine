 ...
 */
export function generateProfile({ location, year, population = 0, countryCode, lat, lon }) {
  ...
  const promptContext = buildPromptContext(location, year, eraMeta, density, climate);
  ...
  // Build micro-events from templates
  const microEvents = templates.map(t => {
    const sources = [];
 ...
 */
function buildPromptContext(location, year, eraMeta, density, climate) {
  const densityDesc = {
  ...
  const parts = [
    ...
    `Climate sounds: ${climateSounds[climate.zone] || 'seasonal ambience'}.`,
  ];
