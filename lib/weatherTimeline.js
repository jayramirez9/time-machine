...
export function selectProvider(provider, useMock, date = null) {
  ...
  if (year < 1940) {
    ...
    const warning = `Pre-1940 date (${year}) requires NOAA_API_TOKEN for real weather data. ` +
      `Using synthetic mock weather. Set NOAA_API_TOKEN env var for historical accuracy.`;
    console.warn(`[WeatherTimeline] ${warning}`);
    return { fn: getWeatherMock, name: 'mock', warning };
  }
