...
// ─── Reference Image Prompt Builder ─────────────────────────

/**
 * Build a prompt for Gemini image generation that produces a clean
 * architectural reference photo. Optimized for Meshy Image-to-3D input:
 * single building, well-lit, 3/4 angle, no clutter.
 *
 * @param {object} opts
 * @param {object} opts.style - classifyBuilding() result
 * @param {object} opts.building - GeoJSON properties (material, use, stories, address)
 * @param {number} opts.year - Target year
 * @param {string} [opts.eraLabel] - Human-readable era label
 * @returns {string} Prompt string for Gemini image generation
 */
export function buildReferenceImagePrompt({ style, building, year, eraLabel }) {
  const parts = [];

  // Photography style instruction
  parts.push('Architectural photograph, three-quarter view from street level, clear daylight, no people or vehicles.');

  // Building identity
  const label = eraLabel || `circa ${year}`;
  const stories = building.stories || 3;
  const useLabel = building.use || 'commercial';
  parts.push(`A ${stories}-story ${style.label || style.styleName} ${useLabel} building, ${label}.`);
  ...
  if (trimMat && trimMat !== primaryMat) {
    ...
    parts.push(`${trimDesc} trim.`);
  }
  const foundationMat = style.materials?.foundation;
  if (foundationMat && foundationMat !== primaryMat) {
    const foundDesc = MATERIAL_DESCRIPTIONS[foundationMat] || foundationMat;
    parts.push(`Foundation: ${foundDesc}.`);
  }

  // Decorative elements — more generous for image gen (5 elements, not 3)
  ...
  if (weathering) parts.push(weathering + '.');

  // Era exclusions as negative guidance
  const exclusions = getEraExclusions(year);
  if (exclusions.length > 0) {
    parts.push(`Do not include: ${exclusions.slice(0, 4).join(', ')}.`);
  }

  // Rendering quality
  parts.push('High detail, photorealistic, sharp focus on facade details.');

  return parts.join(' ');
}

/**
 * Build reference image prompt for a building from its GeoJSON properties.
 * Convenience wrapper that classifies the building first.
 *
 * @param {object} properties - GeoJSON feature properties
 * @param {object} opts - { era, year }
 * @returns {{ prompt: string, style: object, building: object }}
 */
export function buildReferenceImageForBuilding(properties, opts = {}) {
  const { year, era, eraLabel, material, use, stories, style } = _resolveBuilding(properties, opts);
  ...
  return { prompt, style, building: { material, use, stories } };
}

// ─── Shared Preamble ─────────────────────────────────────────

function _resolveBuilding(properties, opts = {}) {
  ...
  return { year, era, eraLabel, material, use, stories, style };
}
