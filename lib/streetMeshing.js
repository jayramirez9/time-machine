...
export function buildStreetSpawnScript(spawnList, opts = {}) {
  ...
  const lines = [
    'import unreal',
    '',
    '# ── Street Layout Spawn Script ──',
    '# Generated by lib/streetMeshing.js',
    '',
  ...
  if (clearExisting) {
    lines.push(
      '# Clear existing street and sidewalk actors',
      'all_actors = unreal.EditorLevelLibrary.get_all_level_actors()',
      'for actor in all_actors:',
      '    label = actor.get_actor_label()',
      `    if label.startswith("${STREET_PREFIX}") or label.startswith("${SIDEWALK_PREFIX}"):`,
      '        actor.destroy()',
      `unreal.log("Cleared existing ${STREET_PREFIX}_* and ${SIDEWALK_PREFIX}_* actors")`,
      ''
    );
  }

  lines.push(
    `# Spawn ${spawnList.length} street elements`,
    'spawned = 0',
    ''
  );
  ...
  for (const s of spawnList) {
    ...
    lines.push(
      `# ${s.label} — ${s.surface}, ${s.widthM}m wide`,
  ...
  lines.push(
    `unreal.log(f"Street layout: spawned {spawned}/${spawnList.length} elements")`,
    ''
  );
