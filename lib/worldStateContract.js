...
export const CONTROL_BOUNDS = {
  ...
  audio: {
    ...
    windDirection: [0, 360],
    // raw air temperature in Celsius — recorded terrestrial extremes are -90/+57
    temperatureC: [-90, 60]
  },
  atmosphere: {
    // cloudCoverage / 100
...
export function validateWorldState(state) {
  ...
  if (typeof state.simMonth === 'number' &&
      (!Number.isInteger(state.simMonth) || state.simMonth < 1 || state.simMonth > 12)) {
    errors.push(`simMonth = ${state.simMonth} must be an integer 1-12`);
  }
