 ...
 *   sonorous         — optional, false = makes no chorus sound (visual-only species);
 *                      such species never reach audio prompts (B048)
 */
...
export function researchEcology({
  ...
  const speciesOutput = species.map(s => {
    ...
    // Preserve the audio-relevance flag — dropping it here silently re-admits
    // silent species to SFX prompts (the B048 firefly path)
    if (s.sonorous === false) entry.sonorous = false;
