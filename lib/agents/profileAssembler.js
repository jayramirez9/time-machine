...
import { researchBuildingDates } from './buildingDateAgent.js';
...
export async function assembleProfile({
  ...
  // Step 4b: Group 2 — agents that depend on Group 1 results
  if (!skip.has('buildingDates') && profile.layers.urbanForm) {
    const bdResult = await runAgent('buildingDates', () =>
      researchBuildingDates({
        ...
        countryCode: geo.countryCode,
        terrainDataPath: resolvedTerrainPath,
        urbanFormLayer: profile.layers.urbanForm,
      }), progress);

    if (bdResult.data) {
      // Merge building inventory into urbanForm layer
      profile.layers.urbanForm.data.buildingInventory = bdResult.data.buildingInventory;
      profile.layers.urbanForm.data.dateCompleteness = bdResult.data.dateCompleteness;
      profile.layers.urbanForm.data.datingMethods = bdResult.data.datingMethods;
      profile.layers.urbanForm.data.buildingsExistingInYear = bdResult.data.buildingsExistingInYear;
    }
  }
