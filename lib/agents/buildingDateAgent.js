...
export function researchBuildingDates({
  ...
  // Precompute location-level evidence (same for all buildings)
  const sanbornEstimate = dateFromSanbornBracket(sanbornCoverage, buildingSource);
  const boomEstimate = dateFromConstructionBoom(location);

  // Step 3: Run evidence methods per building
  const perBuildingEstimates = features.map((feature) => {
    ...
    if (sanbornEstimate) estimates.push(sanbornEstimate);
    ...
    if (boomEstimate) estimates.push(boomEstimate);
