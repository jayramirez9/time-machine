 ...
 * @returns {{ label: string, location: number[], scale: number[], rotation: number[], material: string, stories: number, use: string, address: string }}
 */
export function footprintToSpawnData(feature, origin, index = 0) {
  ...
  const label = `${ACTOR_PREFIX}_${padIdx}_${material}_${stories}s`;
 ...
 */
export function buildingsToSpawnList(geojson, origin) {
  const features = geojson.features || [];
  return features.map((f, i) => footprintToSpawnData(f, origin, i));
}
