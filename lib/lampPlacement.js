...
import {
  ...
  scriptPointLightItem, joinScript
} from './spawnScript.js';
...
export function buildLampSpawnScript(lampList, opts = {}) {
  ...
  const header = scriptHeader('Gas Lamp Spawn Script');

  const clear = clearExisting ? scriptClear(LAMP_PREFIX, 'lamp') : [];

  const counter = scriptCounterStart(lampList.length, 'gas lamps');

  const items = lampList.flatMap(lamp =>
    scriptPointLightItem(lamp, { comment: lamp.label })
  );

  const footer = scriptCounterEnd(lampList.length, 'Gas lamps');
