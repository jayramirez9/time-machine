...
registerTransport('unreal', async (config, params) => {
  ...
  const promises = Object.entries(params).map(async ([paramKey, value]) => {
    ...
    if (actor.type === 'texture') {
      // Import a texture from disk and assign to a material parameter
      // actor config: { sourcePath, contentDir, materialPath, parameterName }
      // Value is ignored (trigger-only) — reads from actor.sourcePath
      if (!actor.sourcePath || !actor.contentDir) {
        console.error(`[dispatch:unreal] texture ${paramKey}: missing sourcePath or contentDir`);
        return;
      }

      const fs = await import('fs');
      const pathMod = await import('path');
      const destDir = pathMod.default.join(actor.contentDir, 'Generated');
      fs.default.mkdirSync(destDir, { recursive: true });
      const destPath = pathMod.default.join(destDir, 'backdrop.png');
      fs.default.copyFileSync(actor.sourcePath, destPath);

      const pythonScript = `
import unreal
source_path = "${destPath.replace(/\\/g, '/')}"
task = unreal.AssetImportTask()
task.set_editor_property("filename", source_path)
task.set_editor_property("destination_path", "/Game/Generated")
task.set_editor_property("destination_name", "backdrop")
task.set_editor_property("replace_existing", True)
task.set_editor_property("automated", True)
task.set_editor_property("save", True)
unreal.AssetToolsHelpers.get_asset_tools().import_asset_tasks([task])
texture = unreal.EditorAssetLibrary.load_asset("/Game/Generated/backdrop")
mat = unreal.EditorAssetLibrary.load_asset("${actor.materialPath || '/Game/Materials/MI_Backdrop'}")
if mat and texture:
    mat.set_texture_parameter_value(unreal.Name("${actor.parameterName || 'BackdropTexture'}"), texture)
    unreal.EditorAssetLibrary.save_asset("${actor.materialPath || '/Game/Materials/MI_Backdrop'}")
`;
      ...
      try {
        ...
        if (!res.ok) {
          const text = await res.text();
          console.error(`[dispatch:unreal] texture ${paramKey}: ${res.status} ${text}`);
        }
      } catch (e) {
        console.error(`[dispatch:unreal] texture ${paramKey}: ${e.message}`);
      }
      return;
    }
