...
export function compileWorldState({
  ...
  // Time-of-day must reflect the scene location's local wall-clock, not the
  // timezone of the machine running the daemon. timestampLocal is the
  // location-local naive time (providers derive it from the location's own
  // timezone); parsing that naive string and reading getHours() yields the
  // scene's local hour identically on any machine. Reading getHours() off the
  // UTC instant instead makes time-of-day machine-timezone-dependent.
  const date = new Date(weather.timestampLocal ?? weather.timestampUtc);
  ...
  // Scene-local calendar month (1-12). Same local-clock reasoning as `hour`:
  // read off timestampLocal so a scene in July is July on any machine.
  // Consumers gate seasonal behavior on it (insect chorus, foliage state).
  // timestampLocal is a provider REQUIREMENT: the UTC fallback above makes
  // the month machine-TZ-dependent at month boundaries (the PR #7 flake class).
  const simMonth = date.getMonth() + 1;
  ...
  return {
    ...
    controls: {
      ...
      audio: {
        ...
        // Raw air temperature, not a normalized control: the micro-event
        // scheduler gates ectotherm choruses on absolute degrees (cicadas need
        // real heat, crickets go silent near freezing), which the `comfort`
        // category is too coarse to express.
