...
/**
 * Detect whether a query string looks like a street address rather than a city name.
 * Matches patterns like "123 Main St", "12877 Erin Ave, Baton Rouge, LA", etc.
 * @param {string} query
 * @returns {boolean}
 */
export function looksLikeAddress(query) {
  ...
  // Starts with a house number followed by a word (e.g. "123 Main")
  if (/^\d+\s+[A-Za-z]/.test(trimmed)) return true;
  // Contains common street type keywords
  if (/\b(street|st|avenue|ave|boulevard|blvd|road|rd|drive|dr|lane|ln|court|ct|place|pl|way|highway|hwy|parkway|pkwy)\b/i.test(trimmed)) return true;
  return false;
}

/**
 * Geocode a street address using OpenStreetMap Nominatim.
 * Returns the same shape as geocode() for interchangeability.
 * @param {string} query - Full street address
 * @returns {Promise<{lat: number, lon: number, name: string, population: number, timezone: string|null, countryCode: string|null}>}
 */
...
/**
 * Smart geocoder: tries Nominatim for street addresses, Open-Meteo for city names.
 * If the primary strategy fails, falls back to the other.
 * @param {string} query - Location string (city name or street address)
 * @returns {Promise<{lat: number, lon: number, name: string, population: number, timezone: string|null, countryCode: string|null}>}
 */
export async function smartGeocode(query) {
  if (looksLikeAddress(query)) {
    // Try Nominatim first for addresses, fall back to Open-Meteo
    try {
  ...
  // City-level query: use Open-Meteo (better metadata), fall back to Nominatim
  try {
    return await geocode(query);
  } catch (_e) {
    return await geocodeAddress(query);
 ...
 * @param {string} [timezone] - IANA timezone for local date calculation
 * @returns {Promise<Object>} Raw API response
 */
// In-memory cache for raw API responses keyed by URL.
// Prevents redundant requests when the timeline fetches multiple hours from the same day.
const _apiCache = new Map();

export async function fetchWeatherData(lat, lon, date, timezone) {
  const apiType = getApiType(date);
  // Use the LOCAL date at the target location for the API query,
  // because timezone=auto returns data indexed by local time.
  // Falling back to UTC date when no timezone is provided.
  ...
  let url;
  if (apiType === 'archive') {
    // Request target local date only. The API returns all 24 hours for this day
    // in the location's local timezone (due to timezone=auto).
  ...
  // Check cache first — timeline fetches multiple hours from the same day
  if (_apiCache.has(url)) {
    return _apiCache.get(url);
  }

  // If another request for this URL is in-flight, wait for it
...
export async function getWeather({ location, date, geo: preGeo }) {
  ...
  // Fetch weather data (pass timezone so archive queries use local date)
