...
export async function getWeather({ location, date }) {
  ...
  // Capture timezone from API response (for future timezone-correct implementation)
  const timezone = data.timezone || 'UTC';
  const timezoneAbbr = data.timezone_abbreviation || 'UTC';

  // Find the hour index for the requested time.
  // Note: data.hourly.time[] contains timestamps in the LOCATION's local timezone
  // (due to timezone=auto). We treat the user's input date as local time at that
  // location, so we match against these local timestamps.
  ...
  // TODO: timestampLocal is currently naive. Proper implementation requires
  // converting the user's intended local time using the location's timezone.
