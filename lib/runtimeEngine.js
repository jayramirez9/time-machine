...
import { setCesiumGeoreference } from './cesiumGeoreference.js';
...
export async function startEngine({
  ...
  // Dispatch CesiumGeoreference if configured and geocode succeeded (fire-and-forget)
  if (routesConfig?.cesiumGeoreference && geo) {
    const geoRefConfig = routesConfig.cesiumGeoreference;
    setCesiumGeoreference({
      lat: geo.lat,
      lon: geo.lon,
      height: geoRefConfig.defaultHeight ?? 0,
      objectPath: geoRefConfig.objectPath,
      host: geoRefConfig.host || routesConfig.endpoints?.unreal?.host
    }).catch(e => console.warn(`[Engine] CesiumGeoreference dispatch failed: ${e.message}`));
  }
  ...
  return {
    ...
    /** Geocoded coordinates { lat, lon, name, timezone } or null */
    get geo() { return geo; },
