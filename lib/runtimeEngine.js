...
import { setGeoreference } from './cesiumGeoreference.js';
...
export async function startEngine({
  ...
  if (routesConfig && geo) {
    ...
    if (unrealEndpoint?.host) {
      const geoResult = await setGeoreference(unrealEndpoint.host, geo.lat, geo.lon);
      if (geoResult.ok) {
        console.log(`[Engine] CesiumGeoreference → ${geo.lat.toFixed(4)}, ${geo.lon.toFixed(4)}`);
      } else {
