 ...
 */
let warnedLegacyTemperature = false;

export function easeWorldState(current, target, rate) {
  if (!current) return target;
  if (current.controls.audio.temperatureC === undefined && !warnedLegacyTemperature) {
    warnedLegacyTemperature = true;
    console.warn('[engine] easing from a state without temperatureC (pre-B047 log/replay?) — snapping to target');
  }
  ...
  return {
    ...
    controls: {
      ...
      audio: {
        ...
        // ?? guard: pre-B047 states (old logs, replays) lack temperatureC
        temperatureC: lerp(current.controls.audio.temperatureC ?? target.controls.audio.temperatureC, target.controls.audio.temperatureC, rate)
      },
      atmosphere: {
  ...
  if (routesConfig && process.env.CESIUM_SPLAT_ASSET_ID && process.env.CESIUM_ION_TOKEN) {
    ...
    if (unrealEndpoint?.host) {
      ...
      if (!Number.isInteger(assetId) || assetId <= 0) {
        // A placeholder like "your-asset-id" parses to NaN — fail with a clear
        // message here instead of a confusing "got NaN" from the script builder.
        const error = `CESIUM_SPLAT_ASSET_ID "${process.env.CESIUM_SPLAT_ASSET_ID}" is not a positive integer`;
        splatResult = { ok: false, assetId: null, error };
        console.warn(`[Engine] Splat tileset skipped: ${error}`);
      } else {
