...
// Dedicated 3DGS splat tileset actor label (Phase 7d.2-A). Lives up here with
// the other constants because findCesium3DTileset() below references it —
// defining it mid-file left a TDZ hazard if that function ever ran during
...
export async function getSplatTilesetStatus(host, { actorLabel = DEFAULT_SPLAT_LABEL } = {}) {
  ...
  let assetId;
  // Deliberate asymmetry: the Python script WRITES via set_editor_property
  // ('ion_asset_id', snake_case — Unreal's Python naming), while the RC HTTP
  // property endpoint READS by the C++ UPROPERTY name ('IonAssetID',
  // PascalCase). Both address the same property.
