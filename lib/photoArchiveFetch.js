...
/**
 * Extract street name tokens from an address string.
 * "56 Broad Street" → ["broad", "street"]
 * "Broadway & Chambers" → ["broadway", "chambers"]
 *
...
/**
 * Score how well a photo matches a building.
 * Higher = better. Returns 0 if no useful signal.
 *
 * @param {object} photo - Photo entry from PHOTO_MANIFEST.json
 * @param {object} building - Building object with { address, styleName }
 * @returns {number} Match score (0 = no match)
 */
function scorePhotoMatch(photo, building) {
  let score = 0;
  const buildingTokens = extractStreetTokens(building.address);
  if (buildingTokens.length === 0) return 0;

  // Check photo title for street name overlap
  const titleTokens = extractStreetTokens(photo.title || '');
  const titleOverlap = buildingTokens.filter(t => titleTokens.includes(t)).length;
  score += titleOverlap * 3;

  // Check photo subjects for street/location overlap
  const subjectText = (photo.subjects || []).join(' ').toLowerCase();
  const subjectTokens = extractStreetTokens(subjectText);
  const subjectOverlap = buildingTokens.filter(t => subjectTokens.includes(t)).length;
  score += subjectOverlap * 2;

  // Bonus: photo title contains an architecture-related term
  const archTerms = ['building', 'facade', 'block', 'row', 'house', 'store', 'church', 'office'];
  if (archTerms.some(t => titleTokens.includes(t))) {
    score += 1;
  }
...
/**
 * Find the best matching photo for a building from a photo manifest.
 *
 * Matching strategy:
 *   1. Score each downloaded photo by street-name overlap with the building address
 *   2. Return the highest-scoring photo above threshold (score >= 2)
 *   3. If no strong match, return the first downloaded photo as a generic era reference
 *   4. If no downloaded photos at all, return null
 *
 * @param {object} manifest - PHOTO_MANIFEST.json contents
 * @param {object} building - Building object with { address, styleName }
 * @param {object} [opts]
 * @param {boolean} [opts.strictMatch=false] - Only return street-matched photos, not generic fallbacks
 * @returns {object|null} Best matching photo entry, or null
 */
export function findBestPhoto(manifest, building, opts = {}) {
  if (!manifest || !Array.isArray(manifest.photos)) return null;

  const downloaded = manifest.photos.filter(p => p.downloaded);
  if (downloaded.length === 0) return null;

  // Score all downloaded photos
  const scored = downloaded
    .map(p => ({ photo: p, score: scorePhotoMatch(p, building) }))
    .sort((a, b) => b.score - a.score);

  // Return best match if score is meaningful (at least 2 = one street word match)
  if (scored[0].score >= 2) {
    return scored[0].photo;
  }

  // Generic fallback: use first downloaded photo as era reference
  if (!opts.strictMatch) {
    return downloaded[0];
  }
